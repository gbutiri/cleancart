...
function process_admin_login () {
    ...
    // 1. check value fields.
    $existingUser = getAdminByEmailAndPassword($email, $password);
    if (count($existingUser) == 0) {
        $err = true;
        $htmls['#username_err'] = "Username or email and password incorrect";
    } else {
    ...
    if ($err) {
        ...
        $_SESSION['site_admin'] = $email;
        $_SESSION['site_user_salt'] = $salt;
        $_SESSION['site_user_token'] = $token;
        
        if (isset($_POST['rememberme']) && $_POST['rememberme'] == 'true') {
            setcookie('site_admin', $_SESSION['site_admin'],(10*365*24*60*60),'/') ;
            setcookie('site_user_salt', $_SESSION['site_user_salt'],(10*365*24*60*60),'/') ;
            setcookie('site_user_token', $_SESSION['site_user_token'],(10*365*24*60*60),'/') ;
        }
        
        $sql_t = "UPDATE admins SET salt = ?, token = ? WHERE email LIKE ?";
        sqlRun($sql_t,'sss',array($salt, $token, $email));
